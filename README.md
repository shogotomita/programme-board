...
作業週は胸・肩・僧帽・二頭・三頭の換算セット（主働1.0・二次0.5）が MAV を超える。各ブロック3週目（3, 7, 11, 15）で肩・僧帽・胸が MRV に達する。四頭は月フロント＋木バックで MEV 以上、後面はデッド＋RDL で MEV〜MAV。種目は日内に固めず週内へ分散し、日毎の総セットも寄せている。4週目はセット半減、RPE 5〜6。
...
ハードセットのみ（ウォームアップ除外）。主働・共主働は1.0、主要二次は0.5。クローズグリップやフロアプレスは胸と三頭の両方に1.0。BJJの引き込みはカウントしていない。ロールがきつい週は補助の最終セットを切る。メインの%は動かさない。
